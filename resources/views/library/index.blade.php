...
<body>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id=package-name name="name"
                                placeholder="vendor/package-name">
                        </div>
                        <div class="form-group m-3">
                            <label for="package-name text-danger" id="error"></label>
                            <label for="package-name text-success" id="success"
                                style="display: none">Successful</label>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <div class="col">
                            <div id="loading" style="display:none">
                                ...
                                <span class="text-success" style="margin-left:10px">Installing <span
                                        id="install-package"></span><br>
                ...
                <tbody>
                    @foreach ($libraries as $key => $library)
                        <tr>
                            ...
                            <td>{{ $library->path }}</td>
                            <td style="display: flex;justify-content: center"><button class="btn btn-danger uninstall" data-name="{{ $library->name }}"
                                    data-search-name="{{ str_replace('/', '', $library->name) }}">Uninstall Library</button>

                                <div class="spinner-border mt-2 mr-3 loading" style="display:none"
                                    data-name="{{ str_replace('/', '', $library->name) }}" role="status">
                                    <span class="sr-only"></span>
                                </div>
                            </td>
                        </tr>
    ...
    <script>
        $(document).on('click', '.uninstall', function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('/api/libraries/uninstall') }}",
                data: {
                    'name': packageName
                },
                success: function(response) {
                    if (response.status == "200" && response.code == 0) {
                        ...
                        $('.uninstall[data-search-name=' + packageNameSearch + ']').css('display',
                            'block');
